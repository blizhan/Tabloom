...
export function fitPreprocessing(dataset: TrainingDataset, config: PreprocessingConfig, bounds = { minRows: 3, maxRows: 1024, maxFeatures: 32 }): FittedState {
  ...
  const orderedColumns = permutation.map((index) => baseColumns[index]); const orderedNames = permutation.map((index) => baseNames[index]); const clip = softClipBounds(orderedColumns);
