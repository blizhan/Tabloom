...
export interface ResultRow { readonly requestId: string; readonly inputSnapshotId: string; readonly rowOrdinal: number; readonly businessKey?: unknown; readonly scenarioId: string; readonly contextKey: string; readonly mean: number; readonly metadata: PredictionResult["metadata"]; readonly createdAt: string; }
export interface RunRecord { readonly requestId: string; readonly streamId: string; readonly generation: number; readonly epoch: string; readonly status: "pending" | "published" | "cancelled" | "superseded" | "failed"; readonly inputSnapshotId: string; readonly scenarioId: string; readonly rows?: readonly ResultRow[]; readonly error?: string; }
...
interface MutableRun extends RunRecord { status: RunRecord["status"]; rows?: readonly ResultRow[]; error?: string; snapshotRetained: boolean; }
...
export class ResultPublicationService {
  ...
  reserve(input: Omit<Reservation, "experiment"> & { experiment?: boolean }): Promise<Reservation> { return this.serial(() => {
    ...
    const run: MutableRun = { requestId: input.requestId, streamId: input.streamId, generation: input.generation, epoch: input.epoch, status: "pending", inputSnapshotId: input.inputSnapshotId, scenarioId: input.scenarioId, snapshotRetained: true };
  ...
  publish(reservation: Reservation, result: PredictionResult, snapshot: InputSnapshot): Promise<readonly ResultRow[]> { return this.serial(() => {
    ...
    if (run.epoch !== reservation.epoch || run.inputSnapshotId !== snapshot.inputSnapshotId || result.requestId !== reservation.requestId || result.inputSnapshotId !== snapshot.inputSnapshotId || result.scenarioId !== reservation.scenarioId || result.contextKey.length === 0 || result.mean.length !== snapshot.rowCount) { run.status = "failed"; run.error = "result identity or row count mismatch"; this.releaseSnapshot(run); throw new RuntimeError("RESULT_INVALID", run.error); }
    ...
    let rows: ResultRow[];
    try { rows = Array.from(result.mean, (mean, rowOrdinal) => { if (!Number.isFinite(mean)) throw new RuntimeError("RESULT_INVALID", `Non-finite prediction at row ${rowOrdinal}`); return { requestId: reservation.requestId, inputSnapshotId: snapshot.inputSnapshotId, rowOrdinal, businessKey: snapshot.businessKeys[rowOrdinal], scenarioId: reservation.scenarioId, contextKey: result.contextKey, mean, metadata: result.metadata, createdAt: new Date().toISOString() }; }); } catch (error) { run.status = "failed"; run.error = error instanceof Error ? error.message : String(error); this.releaseSnapshot(run); throw error; }
    run.status = "published"; run.rows = rows; if (reservation.scenarioId === "baseline" || reservation.generation === 0) this.baseline.set(reservation.streamId, reservation.requestId); return rows;
  }); }
