...
if (!new Set(["fp32", "fp16-storage", "fp8-storage"]).has(variant)) {
...
try {
  ...
  await page.goto(`http://127.0.0.1:4174/?backend=${backend}&variant=${variant}`);
  await page.waitForFunction(() => window.__TABLOOM_RESULT__ !== undefined, null, { timeout: 300_000 });
  const result = await page.evaluate(() => window.__TABLOOM_RESULT__);
  const resultsDir = path.resolve("browser-results");
  await mkdir(resultsDir, { recursive: true });
  const suffix = variant === "fp32" ? "" : `-${variant}`;
  await writeFile(path.join(resultsDir, `${backend}${suffix}.json`), `${JSON.stringify(result, null, 2)}\n`);
